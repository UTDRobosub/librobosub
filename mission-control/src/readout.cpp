...
struct ContinuousPlotGraphic {
    ...
    ContinuousPlotGraphic(const char *figureName_, const char *title_, int numSeries_, float xpos, float ypos,
                          float xsize, float ysize, float xmin_, float xmax_, float ymin_, float ymax_) {
        numSeries = 0;
...
void readout(ReadoutData *data) {
    ...
    int graphHeight = 200 * scale;
    int graphWidth = 600 * scale;

    int curGraphY = -graphHeight;

    //params: identifier title numSeries xpos ypos xsize ysize xmin xmax ymin ymax
    ContinuousPlotGraphic rtt("rtt", "Robot RTT", 1, 0, curGraphY += graphHeight, graphWidth, graphHeight, 0,
                              pointsToGraph, 0, 10);
    ContinuousPlotData rtt_rtt(pointsToGraph);
    rtt.addSeries("RTT ms", rtt_rtt);

    ContinuousPlotGraphic cpu("cpu", "Robot Resource Usage", 1, 0, curGraphY += graphHeight, graphWidth, graphHeight, 0,
                              pointsToGraph, 0, 100);
    ContinuousPlotData cpu_cpu(pointsToGraph);
    cpu.addSeries("CPU %", cpu_cpu);
    ContinuousPlotData cpu_ram(pointsToGraph);
    cpu.addSeries("Memory %", cpu_ram);

    ContinuousPlotGraphic accel("accel", "Acceleration", 3, 0, curGraphY += graphHeight, graphWidth, graphHeight, 0,
                                pointsToGraph, -2, 2);
    ContinuousPlotData accel_x(pointsToGraph);
    accel.addSeries("X G", accel_x);
    ContinuousPlotData accel_y(pointsToGraph);
    accel.addSeries("Y G", accel_y);
    ContinuousPlotData accel_z(pointsToGraph);
    accel.addSeries("Z G", accel_z);

    ContinuousPlotGraphic stuff("stuff", "Stuff", 1, 0, curGraphY += graphHeight, graphWidth, graphHeight, 0,
                                pointsToGraph, 0, 256);
    ContinuousPlotData stuff_stuff(pointsToGraph);
    stuff.addSeries("Stuff", stuff_stuff);
