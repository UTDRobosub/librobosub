...
int main(int argc, char *argv[]) {
    ...
    for (itDesc = typeDesc.begin(); itDesc != typeDesc.end(); ++itDesc) {
        ...
        if (*itDesc == "MSER") {
            if (img.type() == CV_8UC3) {
                b = MSER::create(itMSER->delta, itMSER->minArea, itMSER->maxArea, itMSER->maxVariation,
                                 itMSER->minDiversity, itMSER->maxEvolution,
        ...
        try {
            ...
            if (b.dynamicCast<MSER>() != NULL) {
                ...
                cout << "Number of MSER region " << region.size() << " Number of pixels in all MSER region : "
                     << nbPixelInMSER << "\n";
            }
