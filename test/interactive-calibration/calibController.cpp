...
double calib::calibController::estimateCoverageQuality() {
    ...
    if (mBoardType != chAruco) {
        for (std::vector<std::vector<cv::Point2f> >::iterator it = mCalibData->imagePoints.begin();
             it != mCalibData->imagePoints.end(); ++it)
    ...
    for (std::vector<cv::Mat>::iterator it = mCalibData->allCharucoCorners.begin();
         it != mCalibData->allCharucoCorners.end(); ++it)
...
calib::calibController::calibController(cv::Ptr<calib::calibrationData> data, int initialFlags, bool autoTuning,
...
void calib::calibController::updateState() {
    if (mCalibData->cameraMatrix.total() && mCalibType == Pinhole) {
        ...
        if (sigmaMult * mCalibData->stdDeviations.at<double>(0) / mCalibData->cameraMatrix.at<double>(0, 0) <
            relErrEps &&
            sigmaMult * mCalibData->stdDeviations.at<double>(1) / mCalibData->cameraMatrix.at<double>(1, 1) < relErrEps)
            fConfState = true;
        if (sigmaMult * mCalibData->stdDeviations.at<double>(2) / mCalibData->cameraMatrix.at<double>(0, 2) <
            relErrEps &&
...
double calib::calibDataController::estimateGridSubsetQuality(size_t excludedIndex) {
    {
        ...
        for (size_t k = 0; k < mCalibData->imagePoints.size(); k++)
            if (k != excludedIndex)
                for (std::vector<cv::Point2f>::iterator pointIt = mCalibData->imagePoints[k].begin();
                     pointIt != mCalibData->imagePoints[k].end(); ++pointIt) {
                    int i = (int) ((*pointIt).x / xGridStep);
                    int j = (int) ((*pointIt).y / yGridStep);
                    pointsInCell[i * gridSize + j]++;
                }
...
calib::calibDataController::calibDataController(cv::Ptr<calib::calibrationData> data, int maxFrames,
                                                double convParameter, CalibType calibType) :
...
void calib::calibDataController::filterFrames() {
    ...
    if (numberOfFrames >= mMaxFramesNum) {
        ...
        std::copy(mCalibData->perViewErrors.ptr<double>((int) worstElemIndex + 1),
                  mCalibData->perViewErrors.ptr<double>((int) numberOfFrames),
...
bool calib::calibDataController::saveCurrentCameraParameters() const {
    ...
    if (mCalibData->cameraMatrix.total()) {
        ...
        if (parametersWriter.isOpened()) {
            ...
            parametersWriter << "framesCount" << std::max((int) mCalibData->objectPoints.size(),
                                                          (int) mCalibData->allCharucoCorners.size());
...
void calib::calibDataController::printParametersToConsole(std::ostream &output, CalibType calibType) const {
    ...
    output << "Frames used for calibration: "
           << std::max(mCalibData->objectPoints.size(), mCalibData->allCharucoCorners.size())
    ...
    if (calibType == Pinhole) {
        if (mCalibData->cameraMatrix.at<double>(0, 0) == mCalibData->cameraMatrix.at<double>(1, 1))
            output << "F = " << mCalibData->cameraMatrix.at<double>(1, 1) << " +- "
                   << sigmaMult * mCalibData->stdDeviations.at<double>(1) << std::endl;
        else
            output << "Fx = " << mCalibData->cameraMatrix.at<double>(0, 0) << " +- "
                   << sigmaMult * mCalibData->stdDeviations.at<double>(0) << " \t "
                   << "Fy = " << mCalibData->cameraMatrix.at<double>(1, 1) << " +- "
                   << sigmaMult * mCalibData->stdDeviations.at<double>(1) << std::endl;
        output << "Cx = " << mCalibData->cameraMatrix.at<double>(0, 2) << " +- "
               << sigmaMult * mCalibData->stdDeviations.at<double>(2) << " \t"
               << "Cy = " << mCalibData->cameraMatrix.at<double>(1, 2) << " +- "
               << sigmaMult * mCalibData->stdDeviations.at<double>(3) << std::endl;
        output << "K1 = " << mCalibData->distCoeffs.at<double>(0) << " +- "
               << sigmaMult * mCalibData->stdDeviations.at<double>(4) << std::endl;
        output << "K2 = " << mCalibData->distCoeffs.at<double>(1) << " +- "
               << sigmaMult * mCalibData->stdDeviations.at<double>(5) << std::endl;
        output << "K3 = " << mCalibData->distCoeffs.at<double>(4) << " +- "
               << sigmaMult * mCalibData->stdDeviations.at<double>(8) << std::endl;
        output << "TD1 = " << mCalibData->distCoeffs.at<double>(2) << " +- "
               << sigmaMult * mCalibData->stdDeviations.at<double>(6) << std::endl;
        output << "TD2 = " << mCalibData->distCoeffs.at<double>(3) << " +- "
               << sigmaMult * mCalibData->stdDeviations.at<double>(7) << std::endl;
    } else {
        if (mCalibData->cameraMatrix.at<double>(0, 0) == mCalibData->cameraMatrix.at<double>(1, 1))
    ...
    if (mCalibType == Pinhole) {
        cv::initUndistortRectifyMap(mCalibData->cameraMatrix, mCalibData->distCoeffs, cv::noArray(),
                                    cv::getOptimalNewCameraMatrix(mCalibData->cameraMatrix, mCalibData->distCoeffs,
                                                                  mCalibData->imageSize, 0.0, mCalibData->imageSize),
        ...
        cv::fisheye::initUndistortRectifyMap(mCalibData->cameraMatrix, mCalibData->distCoeffs, cv::noArray(),
                                             P, mCalibData->imageSize, CV_16SC2, mCalibData->undistMap1,
                                             mCalibData->undistMap2);
        ...
        cv::omnidir::initUndistortRectifyMap(mCalibData->cameraMatrix, mCalibData->distCoeffs, mCalibData->omniXi,
                                             cv::noArray(), cv::getOptimalNewCameraMatrix(mCalibData->cameraMatrix,
                                                                                          mCalibData->distCoeffs,
                                                                                          mCalibData->imageSize, 0.0,
                                                                                          mCalibData->imageSize),
                                             mCalibData->imageSize, CV_16SC2, mCalibData->undistMap1,
                                             mCalibData->undistMap2,
