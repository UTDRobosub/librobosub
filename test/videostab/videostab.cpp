...
public:
    ...
    AnalysisResults(vector<cv::Point2f> keypointPrev, vector<cv::Point2f> keypointCur, double analysisTime,
                    unsigned int thread) {
...
public:
    ...
    StabilizationResults(RigidTransform stateEstPre, RigidTransform stateEstPost, RigidTransform measurementResidual,
                         RigidTransform frameCompensation, RigidTransform frameDifference,
                         RigidTransform frameUncorrectedState,
                         float errorCovPre, float errorCovPost, float residCovar, float gain, float stability,
                         double stabilizationTime) {
...
void display(cv::Mat &before, cv::Mat &after, AnalysisResults &analysis, StabilizationResults &stabilization,
    ...
    cv::putText(canvas, cv::String(to_string_with_precision(avgfps())) + cv::String(" FPS using ")
                        + cv::String(to_string_with_precision(curThreads, 0)) + cv::String(" threads"),
                cv::Point(8, canvas.rows - 8),
    ...
    cv::Size textSize = cv::getTextSize(cv::String("Frame ") + cv::String(to_string_with_precision((int) frame, 0))
                                        + cv::String(" of ") +
                                        cv::String(to_string_with_precision((int) frameCount, 0)),
    ...
    cv::putText(canvas, cv::String("Frame ") + cv::String(to_string_with_precision((int) frame, 0))
                ...
                cv::HersheyFonts::FONT_HERSHEY_SIMPLEX, 0.5, CV_RGB(255, 255, 255), 2);
    //Display stability
    cv::putText(canvas, cv::String("Error Covariance: ") +
                        cv::String(to_string_with_precision(stabilization.errorCovariancePosteriori)),
    ...
    textSize = cv::getTextSize(cv::String("Video Size: ") + cv::String(to_string_with_precision(before.cols, 0))
                               ...
                               + cv::String(" @ ") + cv::String(to_string_with_precision(videoFps, 0)) +
                               cv::String(" fps"),
                               cv::HersheyFonts::FONT_HERSHEY_SIMPLEX, 0.5, 2, &baseline);
    ...
    if (csv.is_open()) {
        csv << frame << "," << to_string_with_precision((double) frame / videoFps) << ","
            << to_string_with_precision(analysis.analysisTime) << "," <<
            analysis.thread << "," << stabilization.stabilizationTime << "," << analysis.keypointCur.size() << "," <<
            stabilization.frameDifference.x << "," << stabilization.frameDifference.y << ","
            << stabilization.frameDifference.angle << "," <<
            stabilization.frameUncorrectedState.x << "," << stabilization.frameUncorrectedState.y << ","
            << stabilization.frameUncorrectedState.angle << "," <<
            stabilization.stateEstimatePriori.x << "," << stabilization.stateEstimatePriori.y << ","
            << stabilization.stateEstimatePriori.angle << "," <<
            stabilization.stateEstimatePosteriori.x << "," << stabilization.stateEstimatePosteriori.y << ","
            << stabilization.stateEstimatePosteriori.angle << "," <<
            stabilization.frameCompensation.x << "," << stabilization.frameCompensation.y << ","
            << stabilization.frameCompensation.angle << "," <<
            stabilization.measurementResidual.x << "," << stabilization.measurementResidual.y << ","
            << stabilization.measurementResidual.angle << "," <<
            stabilization.errorCovariancePriori << "," << stabilization.errorCovariancePosteriori << "," <<
            stabilization.residualCovariance << "," << stabilization.optimalKalmanGain << ","
            << stabilization.stabilityIndex << endl;
    }
}
...
static cv::Mat fisheyeCorrection(cv::Mat &src) {
    ...
    initUndistortRectifyMap(intrinsics, dist_coeffs, cv::Mat(), cam, cv::Size(src.cols, src.rows), CV_32FC1, mapx,
                            mapy);
...
stabilizeFrame(const unsigned int frame, RigidTransform frameDiff, RigidTransform frameState, cv::Mat &mat) {
    double _start = CLOCK();
    ...
    RigidTransform stateEstPre = RigidTransform(prediction.at<float>(0, 0),
                                                ...
                                                prediction.at<float>(2, 0));    //predicted state estimate
    ...
    RigidTransform stateEstPost = RigidTransform(target.at<float>(0, 0),
                                                 target.at<float>(1, 0),
                                                 target.at<float>(2, 0));                //a posteriori state estimate
    RigidTransform measurementResidual = RigidTransform(filter.temp5.at<float>(0, 0),
                                                        filter.temp5.at<float>(1, 0),
                                                        filter.temp5.at<float>(2, 0));    //measurement residual
    ...
    return StabilizationResults(stateEstPre, stateEstPost, measurementResidual, frameCompensation, frameDiff,
                                frameState,
...
void stabilizeFrames(const unsigned int frameCount) {
    ...
    while (frameStabilizing < frameCount && running) {
        ...
        if (frameDone) {
            ...
            display(tmp, frames[frameStabilizing % FRAMEBUFFER], dataT[frameStabilizing % FRAMEBUFFER], stabResults,
                    frameStabilizing, frameCount);
