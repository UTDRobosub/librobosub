...
void SerialOnReceive(void* instance, char* message, int length, bool needsresponse, char** response, int* responselength){
	
	for(int i=0; i<length; i++){
		parseSerialInput(message[i]);
	}
}
...
void SerialPollReceive(void* instance){
	while(LOG_PORT.available()){
		char data = LOG_PORT.read();
		Serial_ReceiveChar(serialState, data);
	}
}

void SerialInit() {
	serialState = Serial_NewState(serialInstance, true, SerialOnReceive, SerialSendChar, SerialDelayMs, SerialPollReceive);
}
...
void logIMUData(void)
{
	...
	imuLog.remove(imuLog.length() - 1, 1);
	//imuLog += "]";
	//imuLog += "\r\n"; // Add a new line
	
	Serial_SendMessage(serialState, (char*)imuLog.c_str(), imuLog.length(), false);
